...
class KReporterRESTHandler {
   ...
   function getEnumOptions($path, $grouping = '', $operators = array()) {
      ...
      if ( $moduleArray[0] == 'link' || $moduleArray[0] == 'relate' ) {
         ...
         // Mint start
         if ( empty($fieldType) ) {
            $fieldType = $thisModule->field_defs[$fieldArray[1]]['kreporttype'] ?: $thisModule->field_defs[$fieldArray[1]]['type'];
         }
         switch ( $fieldType ) {
            // Mint end
            case 'enum':
            ...
            case 'name2':
               ...
               while ( $row = $db->fetchByAssoc($result) ) {
                  array_push($returnArray, array( 'value' => $row['field'], 'text' => $row['field'] ));
               }
               break;
            // Mint end
            case 'user_name':
         ...
         // Mint start
         if ( empty($fieldType) ) {
            $fieldType = $parentModule->field_defs[$fieldArray[1]]['kreporttype'] ?: $parentModule->field_defs[$fieldArray[1]]['type'];
         }
