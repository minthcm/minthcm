...
{capture name=getLink assign=link}{{sugarvar key='value'}}{/capture}
    ...
    <iframe id="iframe_{{sugarvar key='name'}}" src="{if $linkStart != 'http://' && $linkStart != 'https:/' && $link}http://{/if}{$link}" title="{if $linkStart != 'http://' && $linkStart != 'https:/' && $link}http://{/if}{$link}" height="{{sugarvar key='height'}}" width="100%"/></iframe>
{/if}
