...
return params;};var params=getQueryParams(document.location.search);params.entryPoint=undefined;if(params.entryPoint!=='Changenewpassword'&&params.module!=='Users'&&params.action!=='Login'){window.location.reload();return true;}
return false;};Alerts.prototype.updateManager=function(){var url='index.php?module=Alerts&action=get&to_pdf=1&view=json';$.ajax(url).done(function(data){if(data==='lost session'||data==='lost password'){Alerts.prototype.redirectToLogin();return false;}
