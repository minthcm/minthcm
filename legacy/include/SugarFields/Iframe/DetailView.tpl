...
{capture name=getLink assign=link}{{sugarvar key='value'}}{/capture}
    ...
    <iframe src="{if $linkStart != 'http://' && $linkStart != 'https:/' && $link}http://{/if}{$link}" title="{if $linkStart != 'http://' && $linkStart != 'https:/' && $link}http://{/if}{$link}" height="{{sugarvar key='height'}}" width="100%" id="{{if empty($displayParams.idName)}}iframe_{{sugarvar key='name'}}{{else}}{{$displayParams.idName}}{{/if}}" /></iframe>
{/if}
