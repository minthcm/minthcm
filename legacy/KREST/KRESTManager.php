...
class KRESTManager
{
    ...
    public function validate_session($session_id)
    {
        if (!empty($session_id)) {
            ...
            if (!empty($_SESSION['is_valid_session']) && 'user' == $_SESSION['type']) {
    ...
    private function login($user_auth)
    {
        ...
        //rrs
        if (!empty($user_auth['encryption']) && 'PLAIN' === $user_auth['encryption'] && "LDAPAuthenticateUser" != $authController->authController->userAuthenticateClass) {
            $user_auth['password'] = md5($user_auth['password']);
        }
        if (!empty($user_auth['encryption']) && 'SPICECRMMOBILE' === $user_auth['encryption']) {
            if ("LDAPAuthenticateUser" != $authController->authController->userAuthenticateClass) {
        ...
        if ($isLoginSuccess) {
            if ('1' == $_SESSION['hasExpiredPassword']) {
        ...
        $success = true;
        } */else if ("LDAPAuthenticateUser" == $authController->authController->userAuthenticateClass && (empty($user_auth['encryption']) || 'PLAIN' == $user_auth['encryption'])) {
