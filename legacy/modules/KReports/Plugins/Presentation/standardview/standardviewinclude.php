...
//function standarddetailviewdisplay(&$thisview) {
    ...
    public function buildFieldArray($thisReport)
    {
        ...
        foreach ($arrayList as $thisList) {
            ...
            if ((isset($thisReport->fieldNameMap[$thisList['fieldid']]['fields_name_map_entry']['type']) && 'currency' == $thisReport->fieldNameMap[$thisList['fieldid']]['fields_name_map_entry']['type']) || (isset($thisReport->fieldNameMap[$thisList['fieldid']]['fields_name_map_entry']['kreporttype']) && 'currency' == $thisReport->fieldNameMap[$thisList['fieldid']]['fields_name_map_entry']['kreporttype'])) {
        ...
        foreach ($arrayList as $thisList) {
            if ('hid' != $thisList['display']) {
                $thisColumn = array(
                    ...
                    'width' => ((isset($thisList['width']) && '' != $thisList['width'] && '0' != $thisList['width']) ? $thisList['width'] : '150'),
                    'sortable' => ('' != $thisList['sort'] && '-' != $thisList['sort'] ? true : false),
                    'dataIndex' => trim($thisList['fieldid'], ':'),
                    'hidden' => ('yes' != $thisList['display'] ? true : false),
