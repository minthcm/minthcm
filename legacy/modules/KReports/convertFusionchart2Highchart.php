...
//echo '<pre>---------------------';
        foreach ($visObj as $layout => $charts) {
            if ('fusioncharts' == $visObj->plugin) {
                $visObj->plugin = 'highcharts';
            }
...
// echo '<pre>fill H '.print_r($visObj->$layout->highcharts, true);
                ...
                if (isset($visObj->$layout->plugin) && 'fusioncharts' == $visObj->$layout->plugin) {
                    $visObj->$layout->plugin = 'highcharts';
                }
                ...
                if ("on" == $visObj->$layout->highcharts->options->legend) {
...
//         echo '<pre>'.print_r($visObj, true);die();
    ...
    {
        if (is_countable($sqls) ? count($sqls) <= 0 : 0) {
