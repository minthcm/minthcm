...
class KReportVisualizationManager
{
    ...
    public function getLayouts()
    {
        ...
        foreach ($this->layouts as $layoutName => $layoutData) {
            $layouts[] = array(
                'name' => $layoutName,
                'count' => is_countable($layoutData['items']) ? count($layoutData['items']) : 0,
            );
        }
    ...
    public function generateLayout($thisLayout, $height)
    {
        ...
        // write the registry
        $layoutString = '';
        if (is_countable($this->pluginRegistry) ? count($this->pluginRegistry) > 0 : 0) {
        ...
        $layout_items_count = is_countable($this->layouts[$thisLayout]['items']) ? count($this->layouts[$thisLayout]['items']) : 0;
        for ($i = 0; $i < $layout_items_count; $i++) {
    ...
    public function updateVisualization($visData, $thisReport, $snaphotId = 0)
    {
        ...
        if ('' != $visData) {
            ...
            if ('-' != $visObject['layout']) {
                $layout_items_count = is_countable($this->layouts[$visObject['layout']]['items']) ? count($this->layouts[$visObject['layout']]['items']) : 0;
                for ($thisElement = 1; $thisElement <= $layout_items_count; $thisElement++) {
