...
//        $this->fill_in_additional_detail_fields();
    ...
    public function save($check_notify = false)
    {
        ...
        if ($this->status != $bean->fetched_row['status'] && 'Held' == $this->status) {
    ...
    public function get_list_view_data()
    {
        ...
        if ($date_db < $today) {
            if ('Held' == $meeting_fields['STATUS'] || 'Not Held' == $meeting_fields['STATUS']) {
    ...
    public function set_notification_body($xtpl, &$meeting)
    {
        ...
        $notifyUser = ('User' == $meetingCurrentNotifyUserObjectName) ?
    ...
    public function set_accept_status(&$user, $status)
    {
        ...
        // MintHCM #54195 Start
        else if ('Candidates' == $user->object_name) {
    ...
    {
        // MintHCM #54195 Start
        if ('Candidates' === $this->parent_type) {
    ...
    /**
     * Parses a bean date field via TimeDate::fromDb (retrying with to_db() normalization
     * if the value isn't already in DB format) and writes the normalized value back to the field.
     * @param string $field property name on $this (e.g. 'date_start')
     * @return SugarDateTime|null
     */
    protected function normalizeDate($field)
    {
        global $timedate;
        $value = $this->$field;
    ...
    protected function updateDateEndOrDuration()
    {
        if (isset($this->date_start) && empty($this->date_end)) {
            $td = $this->normalizeDate('date_start');
            if ($td) {
            ...
            $start = $this->normalizeDate('date_start');
            $end = $this->normalizeDate('date_end');
            if ($start && $end) {
...
function getMeetingsExternalApiDropDown($focus = null, $name = null, $value = null, $view = null)
{
    ...
    if (
        ...
        || "eapm_list" != $dictionaryMeeting['fields']['type']['options']) {
